...
## 差別化ポイント（外販レベルの堀: 日本語×業界別GEO + Share of Voice）

日本語GEOがまだ手薄な今、先行できる武器:

- **Share of Voice（引用シェア）**: 生成AI検索における競合間の引用シェアを配分（`geo_audit/share_of_voice.py`）。
  「自社は業界内で引用シェア何位・何%か」を提示
- **SoV 時系列トラッキング**: スナップショットを蓄積し、引用シェアの推移（up/down/flat）を追える
- API: `POST /v1/share-of-voice`（複数サイト×クエリ群でSoVを算出）
